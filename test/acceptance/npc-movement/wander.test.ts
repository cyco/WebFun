import { describeNPCMovement } from "test/helpers";
import { Char } from "src/engine/objects";
import { rand, Point } from "src/util";

describeNPCMovement(Char.MovementType.Wander, (ctx, tick, vars) => {
	it("moves as expected", async () => {
		const { npc } = vars;
		ctx.engine.metronome.start();

		await tick(".");
		expect(npc.position).toEqual(new Point(5, 3));

		await tick(Array.Repeat(".", 4).join(""));
		expect(npc.position).toEqual(new Point(7, 3));

		await tick(Array.Repeat(".", 12).join(""));
		expect(npc.position).toEqual(new Point(7, 7));

		await tick(Array.Repeat(".", 14).join(""));
		expect(npc.position).toEqual(new Point(7, 1));

		await tick(Array.Repeat(".", 32).join(""));
		expect(npc.position).toEqual(new Point(7, 1));

		await tick(Array.Repeat(".", 16).join(""));
		expect(npc.position).toEqual(new Point(1, 1));

		await tick(Array.Repeat(".", 32).join(""));
		expect(npc.position).toEqual(new Point(7, 7));

		await tick(Array.Repeat(".", 18).join(""));
		expect(npc.position).toEqual(new Point(1, 7));

		await tick(Array.Repeat(".", 20).join(""));
		expect(npc.position).toEqual(new Point(1, 1));

		await tick(Array.Repeat(".", 16).join(""));
		expect(npc.position).toEqual(new Point(7, 1));

		await tick("↑↑↑↑↑");
		await tick(Array.Repeat(".", 4).join(""));
		expect(npc.position).toEqual(new Point(5, 1));

		expect(rand()).toBe(22336);
	});
});
