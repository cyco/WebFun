export default (args, zone, engine) => {
	...
	if (state.currentZone.getTileID(args[0], args[1], 0) === args[2]) return true;
	if (state.currentZone.getTileID(args[0], args[1], 1) === args[2]) return true;
	if (state.currentZone.getTileID(args[0], args[1], 2) === args[2]) return true;
