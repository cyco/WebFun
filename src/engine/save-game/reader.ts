...
class SaveGameReader {
    ...
    read(stream: InputStream) {
        ...
        const seed = stream.getUint32();
        console.log(`Seed: ${seed}`);
        const planet = Planet.FromNumber(stream.getUint32());
        console.log(`Planet: ${planet}`);

        const heroIsOnDagobah = !!stream.getUint32();
        console.log(`Hero is on Dagobah: ${heroIsOnDagobah}`);

        const puzzles1Count = stream.getUint16();
        const puzzleIDs1 = stream.getInt16Array(puzzles1Count);
        console.log(`Puzzles 1: ${puzzleIDs1}`);

        const puzzles2Count = stream.getUint16();
        const puzzleIDs2 = stream.getInt16Array(puzzles2Count);
        console.log(`Puzzles 2: ${puzzleIDs2}`);

        const dagobah = this._readDagobah(stream);
        const world = this._readWorld(stream);

        const inventoryCount = stream.getInt16();
        const inventoryItemIDs = stream.getUint32Array(inventoryCount);
        console.log(`Inventory (${inventoryCount} item${inventoryCount === 1 ? "" : "s"}): ${inventoryItemIDs}`);

        const currentZoneId = stream.getInt16();
        console.log(`Current Zone: ${currentZoneId}`);
        ...
        let currentAmmo = -1;
        if (currentWeapon !== -1)
            currentAmmo = stream.getInt16();
        console.log(`Ammo (current): ${currentAmmo}`);
        const forceAmmo = stream.getInt16();
        console.log(`Ammo (Force): ${forceAmmo}`);
        const blasterAmmo = stream.getInt16();
        console.log(`Ammo (Blaster): ${blasterAmmo}`);
        const blasterRifleAmmo = stream.getInt16();
        console.log(`Ammo (Blaster Rifle): ${currentAmmo}`);
        ...
        const damageTaken = stream.getUint32();
        console.log(`Damage: ${damageTaken}`);
        const livesLeft = stream.getUint32();
        console.log(`Lives: ${livesLeft}`);

        const difficulty = stream.getUint32();
        console.log(`Difficulty: ${difficulty}`);
        const timeElapsed = stream.getUint32();
        console.log(`Time elapsed: ${timeElapsed} second${timeElapsed === 1 ? "" : "s"}`);
        const worldSize = stream.getInt16();
        ...
        console.log(`Goal Puzzle: ${goalPuzzle}`);
        const goalPuzzleAgain = stream.getUint32();
        console.assert(goalPuzzle === goalPuzzleAgain, "Puzzle ID must be the same!");
    }
